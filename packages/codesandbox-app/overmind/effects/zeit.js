...
export default (() => {
    ...
    async function deploysByID(id) {
        try {
            const response = await axios.get(`https://api.zeit.co/v2/now/deployments/${id}/aliases`, {
    ...
    return {
        ...
        getConfig(sandbox) {
            const nowConfigs = sandbox.modules
                .filter(m => m.title === 'now.json' || (m.title === 'package.json' && m.now))
        ...
        async getDeployments(name) {
            const response = await axios.get('https://api.zeit.co/v4/now/deployments', {
                headers: getDefaultHeaders(),
            });
        ...
        async deleteDeployment(id) {
            const response = await axios.delete(`https://api.zeit.co/v9/now/deployments/${id}`, {
                headers: getDefaultHeaders(),
            });
            return response.data.deployments;
        },
        async deploy(contents, sandbox) {
            const apiData = await getApiData(contents, sandbox);
            const deploymentVersion = apiData.version === 2 ? 'v9' : 'v3';
            const response = await axios.post(`https://api.zeit.co/${deploymentVersion}/now/deployments?forceNew=1`, apiData, {
                headers: getDefaultHeaders(),
            });
            return `https://${response.data.url}`;
        },
        async aliasDeployment(id, zeitConfig) {
            const response = await axios.post(`https://api.zeit.co/v2/now/deployments/${id}/aliases`, { alias: zeitConfig.alias }, {
                headers: getDefaultHeaders(),
            });
...
async function getApiData(contents, sandbox) {
