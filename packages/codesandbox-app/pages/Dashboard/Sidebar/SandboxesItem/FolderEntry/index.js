...
import { ARROW_LEFT, ARROW_RIGHT, ESC } from '@codesandbox/common/lib/utils/keycodes';
...
import { DELETE_FOLDER_MUTATION, PATHED_SANDBOXES_CONTENT_QUERY, PATHED_SANDBOXES_FOLDER_QUERY, RENAME_FOLDER_MUTATION, } from '../../../queries';
        ...
        return connectDropTarget(connectDragSource(<div>
          <ContextMenu items={menuItems}>
            ...
            ...(decodeURIComponent(currentPath) === path &&
              ...
              <IconContainer>
                {toToggle ? (<AnimatedChevron onClick={this.toggleOpen} open={this.state.open} style={{ opacity: children.size > 0 ? 1 : 0 }}/>) : null}
                <FolderIcon />
              </IconContainer>{' '}
          ...
          <ReactShow show={children.size > 0 && !isDragging && this.state.open && toToggle} duration={250} stayMounted={false} style={{
            height: 'auto',
            overflow: 'hidden',
            ...
            return (<DropFolderEntry path={childPath} selectedSandboxes={selectedSandboxes} url={childUrl} basePath={basePath} teamId={teamId} folders={folders} foldersByPath={foldersByPath} key={childName} name={childName} depth={this.props.depth + 1} open={currentPath.indexOf(childPath) === 0} onSelect={onSelect} currentPath={currentPath} currentTeamId={currentTeamId}/>);
        })}
          </ReactShow>
          {this.state.creatingDirectory && (<CreateFolderEntry teamId={teamId} depth={this.props.depth} close={() => this.setState({ creatingDirectory: false })} basePath={path}/>)}
        </div>));
    }
}
