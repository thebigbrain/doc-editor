...
export default class Module {
    ...
    _compile(content, filename) {
        ...
        global.global = self;
        Module.evaluate(global, [content, `//# sourceURL=${location.origin}${filename}`].join('\n'));
    }
    static evaluate(global, content) {
        ...
        const result = (0, eval)(newCode).apply(this, Object.keys(global).map(m => global[m]));
        self.define = oldamd;
        return result;
    }
    ...
    static _load(request, parent, isMain) {
        ...
        if (request === 'fs' || request === 'graceful-fs') {
            ...
            fs.accessSync = (_path, _type) => {
                if (fs.existsSync(_path)) {
                    return true;
                }
                throw new Error('File not found');
            };
            fs.access = (_path, cb1, cb) => {
