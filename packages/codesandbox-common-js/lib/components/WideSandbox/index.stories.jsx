...
const sandboxWithKnobs = (group, sandbox) => ({
    ...
    template: select('template', fixtures.templateOptions, sandbox.template, group),
    tags: array('tags', sandbox.tags, ',', group),
});
const createSandboxStory = ({ sandbox = fixtures.sandbox(), selectSandbox = action('selectSandbox'), small, noHeight, defaultHeight, noMargin, }) => () => (<WideSandbox sandbox={sandboxWithKnobs('sandbox', sandbox)} selectSandbox={selectSandbox} small={boolean('small', small)} noHeight={boolean('noHeight', noHeight)} defaultHeight={number('defaultHeight', defaultHeight)} noMargin={boolean('noMargin', noMargin)}/>);
