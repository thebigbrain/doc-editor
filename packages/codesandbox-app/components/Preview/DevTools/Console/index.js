...
import { ConsoleInput } from './Input/index';
...
class ConsoleComponent extends React.Component {
    ...
    render() {
        ...
        <Messages ref={el => {
          ...
          <ConsoleFeed logs={messages} variant={this.props.theme.light ? 'light' : 'dark'} styles={inspectorTheme(this.props.theme)} filter={filter} searchKeywords={searchKeywordsHasError ? '' : searchKeywords}/>
        </Messages>
        <ConsoleInput evaluateConsole={this.evaluateConsole}/>
      </Container>);
    }
}
const ConsoleFilterInput = ({ style }) => (<FilterInput placeholder="Filter" style={style} onChange={({ target: { value } }) => dispatch({ type: 'search-log', value })}/>);
...
export const console = {
    ...
    Content: withTheme(ConsoleComponent),
    actions: [
        {
            title: 'Clear Console',
            onClick: () => {
