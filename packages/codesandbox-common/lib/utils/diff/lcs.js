...
var LcsDiff = /** @class */ (function () {
  ...
  LcsDiff.prototype.ElementsAreEqual = function (originalIndex, newIndex) {
    return (this.OriginalSequence.getElementAtIndex(originalIndex) ===
      this.ModifiedSequence.getElementAtIndex(newIndex))
  }
  LcsDiff.prototype.OriginalElementsAreEqual = function (index1, index2) {
    return (this.OriginalSequence.getElementAtIndex(index1) ===
      this.OriginalSequence.getElementAtIndex(index2))
  }
  LcsDiff.prototype.ModifiedElementsAreEqual = function (index1, index2) {
    return (this.ModifiedSequence.getElementAtIndex(index1) ===
  ...
  LcsDiff.prototype.ComputeRecursionPoint = function (originalStart, originalEnd, modifiedStart, modifiedEnd, midOriginalArr, midModifiedArr, quitEarlyArr) {
    ...
    for (numDifferences = 1; numDifferences <= maxDifferences / 2 + 1; numDifferences++) {
      ...
      var matchLengthOfLongest = (furthestOriginalIndex -
        originalStart +
  ...
  LcsDiff.prototype.ShiftChanges = function (changes) {
    ...
    do {
      ...
      for (var i = 0; i < changes.length; i++) {
        ...
        var originalStop = i < changes.length - 1
          ? changes[i + 1].originalStart
          : this.OriginalSequence.getLength()
        var modifiedStop = i < changes.length - 1
  ...
  LcsDiff.prototype._OriginalRegionIsBoundary = function (originalStart, originalLength) {
    if (this._OriginalIsBoundary(originalStart) ||
      this._OriginalIsBoundary(originalStart - 1)) {
      return true
    }
    if (originalLength > 0) {
      var originalEnd = originalStart + originalLength
      if (this._OriginalIsBoundary(originalEnd - 1) ||
  ...
  LcsDiff.prototype._ModifiedRegionIsBoundary = function (modifiedStart, modifiedLength) {
    if (this._ModifiedIsBoundary(modifiedStart) ||
      this._ModifiedIsBoundary(modifiedStart - 1)) {
      return true
    }
    if (modifiedLength > 0) {
      var modifiedEnd = modifiedStart + modifiedLength
      if (this._ModifiedIsBoundary(modifiedEnd - 1) ||
  ...
  LcsDiff.prototype._boundaryScore = function (originalStart, originalLength, modifiedStart, modifiedLength) {
    var originalScore = this._OriginalRegionIsBoundary(originalStart, originalLength)
      ? 1
      : 0
    var modifiedScore = this._ModifiedRegionIsBoundary(modifiedStart, modifiedLength)
      ? 1
      : 0
