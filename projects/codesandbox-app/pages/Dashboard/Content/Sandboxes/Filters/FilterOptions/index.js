...
const FilterOptionsComponent = ({ possibleTemplates, hideFilters, store, signals, }) => {
    ...
    const allSelected = possibleTemplates.every(t => store.dashboard.isTemplateSelected(t.id));
    const Overlay = () => (<OverlayContainer>
      {possibleTemplates.length > 0 ? (<>
        ...
        return (<Option toggleTemplate={toggleTemplate} selected={selected} key={template.name} color={template.color} id={template.id} niceName={template.niceName || template.name}/>);
    })}
...
export const FilterOptions = inject('store', 'signals')(observer(FilterOptionsComponent));
