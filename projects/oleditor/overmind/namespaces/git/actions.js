...
export const repoTitleChanged = ({ state }, { title }) => {
    state.git.repoTitle = title;
    state.git.error = null;
};
export const createRepoClicked = async ({ state, effects }) => {
...
export const gitMounted = ({ actions }) => actions.git.internal.fetchGitChanges();
export const createCommitClicked = async ({ state, effects }) => {
    const { git } = state;
    const id = state.editor.currentId;
    git.commit = null;
    git.isCommitting = true;
    state.currentModal = 'commit';
    const commit = await effects.api.createGitCommit(id, `${git.subject}${git.description.length ? `\n\n${git.description}` : ``}`);
...
export const subjectChanged = ({ state }, { subject }) => {
    state.git.subject = subject;
};
export const descriptionChanged = ({ state }, { description }) => {
    state.git.description = description;
};
export const createPrClicked = async ({ state, effects }) => {
    state.git.pr = null;
    state.git.isCreatingPr = true;
    state.currentModal = 'pr';
    const id = state.editor.currentId;
    const pr = await effects.api.createGitPr(id, `${state.git.subject}${state.git.description.length ? `\n\n${state.git.description}` : ``}`);
    state.git.pr = pr;
