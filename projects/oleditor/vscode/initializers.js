...
export function initializeCustomTheme() {
    ...
    if (customTheme) {
        ...
        // @ts-ignore
        const fs = BrowserFS.BFSRequire('fs');
        fs.writeFileSync('/extensions/custom-theme/package.json', JSON.stringify(packageJSON));
        fs.mkdirSync('/extensions/custom-theme/themes');
        fs.writeFileSync('/extensions/custom-theme/themes/custom-color-theme.json', JSON.parse(customTheme));
    }
}
const VIM_EXTENSION_ID = 'vscodevim.vim';
export function setVimExtensionEnabled(vimEnabled) {
    if (vimEnabled) {
        vscode.enableExtension(VIM_EXTENSION_ID);
    }
    else {
        // Auto disable vim extension
        if ([null, undefined].includes(localStorage.getItem('vs-global://extensionsIdentifiers/disabled'))) {
            localStorage.setItem('vs-global://extensionsIdentifiers/disabled', '[{"id":"vscodevim.vim"}]');
        }
