...
export default {
    save(currentId, version, module, code, savedCode) {
        try {
            localStorage.setItem(getKey(currentId, module.shortid), JSON.stringify({
                code,
                savedCode,
                version,
    ...
    getRecoverList(currentId, modules) {
        const localKeys = Object.keys(localStorage).filter(key => key.startsWith(`recover:${currentId}`));
        return modules
