...
class DevToolTerminal extends React.Component {
    ...
    render() {
        ...
        {!hidden && owned && (<ShellTabs selectedShell={this.state.selectedShell} shells={this.state.shells} selectShell={this.selectShell} closeShell={this.closeShell} createShell={this.createShell}/>)}

        <div style={{ position: 'relative', flex: 'auto' }}>
          <TerminalComponent hidden={hidden || selectedShell !== undefined} theme={theme} onTerminalInitialized={this.setTerminal}/>
          {this.state.shells.map(shell => (<Shell key={shell.id} id={shell.id} script={shell.script} ended={shell.ended} hidden={hidden || shell.id !== this.state.selectedShell} closeShell={() => this.closeShell(shell.id)} endShell={() => this.endShell(shell.id)}/>))}
        </div>
      </div>);
    }
}
