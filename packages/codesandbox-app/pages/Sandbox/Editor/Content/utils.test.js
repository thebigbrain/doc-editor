import { addDevToolsTab, moveDevToolsTab } from './utils';
...
describe('moveDevToolsTab', () => {
    it('can move a tab in same devtools', () => {
        ...
        const newTabs = moveDevToolsTab(tabs, { devToolIndex: 0, tabPosition: 0 }, { devToolIndex: 0, tabPosition: 1 });
...
describe('addDevToolsTab', () => {
    ...
    it('can add a tab on specific position', () => {
        const tabs = [{ views: [{ ...BROWSER_FIXTURE }, { ...TESTS_FIXTURE }] }];
        expect(addDevToolsTab(tabs, TESTS_FIXTURE, { devToolIndex: 0, tabPosition: 0 })).toMatchSnapshot();
    });
});
