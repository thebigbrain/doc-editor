...
export const deployWithNetlify = async ({ effects, actions, state, }) => {
    ...
    try {
        const id = await effects.netlify.deploy(zip.file, state.editor.currentSandbox);
        ...
        effects.notificationToast.error('An unknown error occurred when deploying your site');
    }
    state.deployment.deploying = false;
    state.deployment.building = false;
};
export const getNetlifyDeploys = async ({ state, effects }) => {
    try {
        state.deployment.netlifyClaimUrl = await effects.netlify.claimSite(state.editor.currentId);
        state.deployment.netlifySite = await effects.netlify.getDeployments(state.editor.currentId);
    }
    catch (error) {
        state.deployment.netlifySite = null;
    }
};
export const getDeploys = async ({ state, effects }) => {
    ...
    try {
        ...
        state.deployment.sandboxDeploys = await effects.zeit.getDeployments(zeitConfig.name);
    }
    catch (error) {
        effects.notificationToast.error('An unknown error occurred when connecting to ZEIT');
    }
    state.deployment.gettingDeploys = false;
};
export const deployClicked = async ({ state, effects, actions, }) => {
    state.deployment.deploying = true;
    const zip = await effects.zip.create(state.editor.currentSandbox);
    const contents = await effects.jsZip.loadAsync(zip.file);
    try {
        state.deployment.url = await effects.zeit.deploy(contents, state.editor.currentSandbox);
    }
    catch (error) {
        effects.notificationToast.error('An unknown error occurred when connecting to ZEIT');
    }
...
export const deploySandboxClicked = async ({ state, effects }) => {
    state.currentModal = 'deployment';
    const zeitIntegration = state.user.integrations.zeit;
    if (!zeitIntegration || !zeitIntegration.token) {
        effects.notificationToast.error('You are not authorized with Zeit, please refresh and log in again');
        return;
    }
...
export const setDeploymentToDelete = ({ state }, { id }) => {
    state.deployment.deployToDelete = id;
};
export const deleteDeployment = async ({ state, effects, actions, }) => {
    ...
    try {
        ...
        effects.notificationToast.error('An unknown error occurred when deleting your deployment');
    }
    state.deployment.deploysBeingDeleted.splice(state.deployment.deploysBeingDeleted.indexOf(id), 1);
};
export const aliasDeployment = async ({ state, effects, actions }, { id }) => {
    const zeitConfig = effects.zeit.getConfig(state.editor.currentSandbox);
    try {
        ...
        effects.notificationToast.error('An unknown error occurred when aliasing your deployment');
    }
};
