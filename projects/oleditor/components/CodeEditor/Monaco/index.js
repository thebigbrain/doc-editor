...
class MonacoEditor extends React.Component {
  ...
  setupTypeWorker = () => {
    ...
    if (this.typingsFetcherWorker) {
      this.typingsFetcherWorker.addEventListener('message', event => {
        ...
        Object.keys(event.data).forEach((path) => {
  ...
  updateDecorations = async (classifications) => {
  ...
  swapDocuments = (currentModule, nextModule) => {
  ...
  fetchDependencyTypings = (dependencies) => {
    if (this.typingsFetcherWorker) {
      this.monaco.languages.typescript.typescriptDefaults.setDiagnosticsOptions(
        {
  ...
  getModelByShortid = async (shortid) => {
