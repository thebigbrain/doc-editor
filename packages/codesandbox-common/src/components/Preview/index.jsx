...
import {actions, dispatch, listen, registerFrame, resetState,} from 'codesandbox-api'
...
class BasePreview extends React.Component {
  ...
  render() {
    ...
    return (<Container className={className} style={{
      ...
      {showNavigation && (
        <Navigator url={url} onChange={this.updateUrl} onConfirm={this.sendUrl} onBack={back ? this.handleBack : null}
                   onForward={forward ? this.handleForward : null} onRefresh={this.handleRefresh}
                   isProjectView={isInProjectView}
                   toggleProjectView={this.props.onToggleProjectView && this.toggleProjectView}
                   openNewWindow={this.openNewWindow} zenMode={settings.zenMode}/>)}
      ...
      <AnySpring from={{opacity: 0}} to={{
        ...
        {(style) => (<>
          <StyledFrame
            ...
            src={this.currentUrl()} ref={this.setIframeElement} title={getSandboxName(sandbox)} id="sandbox-preview"
            style={{
              ...style,
              zIndex: 1,
