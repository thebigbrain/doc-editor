...
var isHttp = Boolean(window.location.protocol === 'http:');

function register(swUrl, _ref) {
  var onUpdated = _ref.onUpdated,
      onInstalled = _ref.onInstalled;
...
function registerValidSW(swUrl) {
  var _ref2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      onUpdated = _ref2.onUpdated,
      onInstalled = _ref2.onInstalled;
        ...
        if (installingWorker.state === 'installed') {
          ...
          if ('storage' in navigator && 'estimate' in navigator.storage) {
            navigator.storage.estimate().then(function (results) {
              var percentUsed = results.usage / results.quota * 100; // Let's assume that if we're using 95% of our quota, then this failure
              ...
              if (percentUsed >= 0.95) {
                ...
                registration.unregister(); // Let's assume that we have some way of doing this without inadvertantly
          ...
          } else {// What about browsers that don't support navigator.storage.estimate()?
...
function checkValidServiceWorker(swUrl, _ref3) {
  var onUpdated = _ref3.onUpdated,
      onInstalled = _ref3.onInstalled;
