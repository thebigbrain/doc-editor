...
function initializeRequires() {
  ...
  global.require.define('vs/base/common/amd', [], () => ({
    getPathFromAmdModule: (_, relativePath) => require('path').join('/vs', relativePath),
  }))
...
export default function(isVSCode = false, requiredModule) {
  ...
  if (IS_FILE_PROTOCOL) {
    ...
    DIRNAME =
      ...
      port +
      global.location.pathname.substr(0, global.location.pathname.lastIndexOf('/'))
  ...
  Component.prototype.renderLoadingOptions = function() {
    return ('<strong style="width:130px;display:inline-block;">' +
      this.name +
      '</strong>:&nbsp;&nbsp;&nbsp;' +
  ...
  return function(callback, PATH_PREFIX) {
      ...
      if (requiredModule) {
        global.require(requiredModule, function(a) {
          if (!isVSCode && !RESOLVED_CORE.isRelease()) {
            ...
            global.require(RESOLVED_PLUGINS.map(function(plugin) {
    ...
    if (global.require) {
      ...
      loadScript(PATH_PREFIX + RESOLVED_CORE.getResolvedPath() + '/loader.js', loadFiles)
    }
  }
}
