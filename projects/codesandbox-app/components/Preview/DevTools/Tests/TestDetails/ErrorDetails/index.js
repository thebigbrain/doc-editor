...
`;
const formatDiffMessage = (error, path) => {
    let finalMessage = '';
    if (error.matcherResult) {
        finalMessage = `<span>${escapeHtml(error.message)
            ...
            .replace(/(Expected.*\n)(.*)/m, `<span>$1</span><span style="color:${theme.green()}">$2</span>`)
            .replace(/(Received.*\n)(.*)/m, `<span>$1</span><span style="color:${theme.red()}">$2</span>`)
    ...
    if (error.mappedErrors &&
        ...
        const widestNumber = Math.max(...mappedError._originalScriptCode.map(code => (code.lineNumber + '').length)) + 2;
...
export const ErrorDetails = ({ error, path, }) => (<Container dangerouslySetInnerHTML={{
