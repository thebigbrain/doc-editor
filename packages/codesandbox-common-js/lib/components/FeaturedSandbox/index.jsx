...
import { Container, SandboxContainer, SandboxInfo, Title, Description, Author, IconContainer, StyledStats, SandboxPreviewImage, } from './elements';
...
export default class FeaturedSandbox extends React.PureComponent {
    ...
    render() {
        ...
        <SandboxContainer role="button" onClick={this.toggleOpen}>
          <SandboxInfo>
            ...
            {sandbox && (<Springg from={{ height: 0, opacity: 0, overflow: 'hidden' }} to={{ height: 28, opacity: 1 }}>
                {style => (<StyledStats style={style} viewCount={sandbox.viewCount} likeCount={sandbox.likeCount} forkCount={sandbox.forkCount}/>)}
              </Springg>)}
          </SandboxInfo>
        ...
        {typeof window === 'undefined' ? (<div style={{ flex: 1, opacity: 1 }}>
          ...
          </div>) : (<Transitionn items={this.state.showPreview} from={{ flex: 1, opacity: 1 }} enter={{ opacity: 1, flex: 1 }} leave={{
            opacity: 0,
            flex: 1,
            position: 'absolute',
            top: 0,
