...
var is_babel_7_1 = require("../../../utils/is-babel-7");
var JSX_PRAGMA = {
...
var config = {
    ...
    getDefaultCode: function (template, resolveModule) {
        var isV7 = false;
        try {
        ...
        if (template === 'parcel') {
            ...
            var parserOpts = isV7 ? { plugins: ['dynamicImport'] } : {};
            var packageJSONModule = resolveModule('/package.json');
            if (packageJSONModule) {
                try {
                    var parsed_1 = JSON.parse(packageJSONModule.code);
                    var pragma_1 = null;
                    Object.keys(JSX_PRAGMA).forEach(function (dep) {
                        if ((parsed_1.dependencies && parsed_1.dependencies[dep]) ||
                            (parsed_1.devDependencies && parsed_1.devDependencies[dep])) {
                            pragma_1 = JSX_PRAGMA[dep];
                        }
                    });
                    if (pragma_1 !== null) {
                        // @ts-ignore
                        plugins.push(['transform-react-jsx', { pragma: pragma_1 }]);
                    }
                }
                catch (e) {
                    /* do nothing */
            ...
            return JSON.stringify({ presets: presets, plugins: plugins, parserOpts: parserOpts }, null, 2);
        }
        if (template === 'cxjs') {
            if (isV7) {
                return JSON.stringify({
