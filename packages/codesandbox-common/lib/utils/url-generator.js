...
export const newCxJSSandboxUrl = () => `/s/github/codaxy/cxjs-codesandbox-template`
...
export const sandboxUrl = (sandboxDetails) => {
...
export const frameUrl = (sandbox, append = '', useFallbackDomain = false) => {
...
export const signInUrl = (extraScopes = false) => '/auth/github' + (extraScopes ? '?scope=user:email,public_repo' : '')
export const signInZeitUrl = () => '/auth/zeit'
export const profileUrl = (username) => `/u/${username}`
export const dashboardUrl = () => `/dashboard`
export const exploreUrl = () => `/explore`
export const teamOverviewUrl = teamId => `/dashboard/teams/${teamId}`
export const profileSandboxesUrl = (username, page) => `${profileUrl(username)}/sandboxes${page ? `/${page}` : ''}`
export const profileLikesUrl = (username, page) => `${profileUrl(username)}/likes${page ? `/${page}` : ''}`
export const githubRepoUrl = ({repo, branch, username, path,}) => buildEncodedUri`https://github.com/${username}/${repo}/tree/${branch}/` +
  path
...
export const gitHubToSandboxUrl = (githubUrl) => githubUrl.replace(gitHubPrefix, '/s/github').replace(dotGit, '')
